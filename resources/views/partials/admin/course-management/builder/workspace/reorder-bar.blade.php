<div x-show="reorderMode" x-cloak class="space-y-4 rounded-2xl border-2 border-amber-300 bg-amber-50/60 p-4 sm:p-5 shadow-sm">
    ...
    {{-- Error Banner --}}
    <div x-show="reorderError" x-cloak class="rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-700 font-medium flex items-center justify-between">
        <span x-text="reorderError"></span>
        <button type="button" @click="reorderError = null" class="font-bold text-red-800 hover:underline">Dismiss</button>
    </div>
        ...
        <template x-for="(item, idx) in reorderItems" :key="item.id">
            <div
                ...
                <div class="flex items-center gap-3">
                    {{-- Drag Handle --}}
                    <span class="text-amber-400 hover:text-amber-600 p-1" title="Drag to reorder" aria-label="Drag handle">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                        </svg>
                    </span>

                    <span class="flex h-6 w-6 items-center justify-center rounded-md bg-amber-100 text-xs font-black text-amber-900" x-text="idx + 1"></span>
                ...
                <div class="flex items-center gap-1">
                    <button
                        ...
                        class="rounded p-1 text-slate-400 hover:bg-amber-100 hover:text-amber-800 disabled:opacity-30 disabled:hover:bg-transparent">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="flex items-center justify-between border-t border-amber-200/80 pt-3">
        ...
        <button
            ...
            @click="saveReorder()"
            :disabled="reorderSaving"
            class="inline-flex items-center gap-1.5 rounded-xl bg-amber-600 px-5 py-2 text-xs font-bold text-white shadow-sm hover:bg-amber-700 transition disabled:opacity-50">
