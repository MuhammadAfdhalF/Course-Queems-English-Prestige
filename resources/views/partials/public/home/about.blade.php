<section class="bg-white">
    <div class="mx-auto grid max-w-7xl gap-10 px-4 py-16 lg:grid-cols-[1.05fr_1fr] lg:px-8">
        <div class="max-w-xl">
            <h2 class="text-2xl font-bold leading-tight text-[var(--color-brand-navy)] md:text-3xl">
                About <span class="text-[var(--color-brand-gold)]">Queens English Prestige</span>
            </h2>

            <div class="mt-5 space-y-5 text-sm leading-8 text-slate-600 md:text-base">
                <p>
                    Queens English Prestige provides premium English learning through
                    online courses and offline sessions, designed for learners who want
                    real progress in speaking, grammar, and test preparation. Whether
                    you prefer learning independently through modules or studying in a
                    classroom setting, we offer a clear learning path with measurable
                    results.
                </p>

                <p>
                    Choose your program, select Online or Offline, and complete your order
                    through our website. Our team will confirm the details via WhatsApp
                    to get you started quickly.
                </p>
            </div>

            <div class="mt-7">
                <x-ui.button class="bg-[var(--color-brand-blue)] hover:opacity-90">
                    Learn More
                </x-ui.button>
            </div>
        </div>

        <div class="grid gap-x-10 gap-y-8 sm:grid-cols-2">
            <div class="flex items-start gap-3">
                <div class="mt-0.5 flex h-8 w-8 items-center justify-center rounded-md bg-[var(--color-brand-blue-soft)] text-[var(--color-brand-blue)]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 8v4l3 3" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </div>
                <div>
                    <h3 class="text-sm font-bold text-[var(--color-brand-navy)] md:text-base">
                        Online & Offline Options
                    </h3>
                    <p class="mt-1 text-xs leading-6 text-slate-500 md:text-sm">
                        Learn through online modules or join guided offline sessions.
                    </p>
                </div>
            </div>

            <div class="flex items-start gap-3">
                <div class="mt-0.5 flex h-8 w-8 items-center justify-center rounded-md bg-[var(--color-brand-blue-soft)] text-[var(--color-brand-blue)]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 18h.01M8 21h8M7 3h10a2 2 0 012 2v10a2 2 0 01-2 2H7a2 2 0 01-2-2V5a2 2 0 012-2z" />
                    </svg>
                </div>
                <div>
                    <h3 class="text-sm font-bold text-[var(--color-brand-navy)] md:text-base">
                        Speaking-Focused Practice
                    </h3>
                    <p class="mt-1 text-xs leading-6 text-slate-500 md:text-sm">
                        Practical drills to build fluency, pronunciation, and confidence.
                    </p>
                </div>
            </div>

            <div class="flex items-start gap-3">
                <div class="mt-0.5 flex h-8 w-8 items-center justify-center rounded-md bg-[var(--color-brand-blue-soft)] text-[var(--color-brand-blue)]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 14l4-4 4 4 8-8" />
                    </svg>
                </div>
                <div>
                    <h3 class="text-sm font-bold text-[var(--color-brand-navy)] md:text-base">
                        Structured Learning Path
                    </h3>
                    <p class="mt-1 text-xs leading-6 text-slate-500 md:text-sm">
                        Clear levels, milestones, and a step-by-step curriculum.
                    </p>
                </div>
            </div>

            <div class="flex items-start gap-3">
                <div class="mt-0.5 flex h-8 w-8 items-center justify-center rounded-md bg-[var(--color-brand-blue-soft)] text-[var(--color-brand-blue)]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 17v-5a2 2 0 012-2h4" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M13 7h4v4" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M5 19h14" />
                    </svg>
                </div>
                <div>
                    <h3 class="text-sm font-bold text-[var(--color-brand-navy)] md:text-base">
                        Exercises & Progress Tracking
                    </h3>
                    <p class="mt-1 text-xs leading-6 text-slate-500 md:text-sm">
                        Practice after each lesson with measurable progress.
                    </p>
                </div>
            </div>

            <div class="flex items-start gap-3">
                <div class="mt-0.5 flex h-8 w-8 items-center justify-center rounded-md bg-[var(--color-brand-blue-soft)] text-[var(--color-brand-blue)]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 12l2 2 4-4" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 3l7 4v5c0 5-3.5 8-7 9-3.5-1-7-4-7-9V7l7-4z" />
                    </svg>
                </div>
                <div>
                    <h3 class="text-sm font-bold text-[var(--color-brand-navy)] md:text-base">
                        Final Assessment & Certificate
                    </h3>
                    <p class="mt-1 text-xs leading-6 text-slate-500 md:text-sm">
                        Complete the course, pass the exam, and earn your certificate.
                    </p>
                </div>
            </div>

            <div class="flex items-start gap-3">
                <div class="mt-0.5 flex h-8 w-8 items-center justify-center rounded-md bg-[var(--color-brand-blue-soft)] text-[var(--color-brand-blue)]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-4l-4 4v-4z" />
                    </svg>
                </div>
                <div>
                    <h3 class="text-sm font-bold text-[var(--color-brand-navy)] md:text-base">
                        Easy Enrollment via WhatsApp
                    </h3>
                    <p class="mt-1 text-xs leading-6 text-slate-500 md:text-sm">
                        Fast confirmation for online or offline orders and scheduling.
                    </p>
                </div>
            </div>
        </div>
    </div>
</section>