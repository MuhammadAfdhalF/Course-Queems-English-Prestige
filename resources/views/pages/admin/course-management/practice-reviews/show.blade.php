...
'pageTitle' => 'Review Practice Attempt',
'pageSubtitle' => $practice->title,
])
...
<section class="mx-auto max-w-6xl space-y-6">
    <x-admin.page-toolbar
        :back-url="route('admin.course-management.practice-reviews.index', $practice)"
        ...
        <div class="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-start">
            <div>
                ...
                <p class="mt-4 text-sm leading-6 text-slate-600">
                    {{ $practice->module->courseLevel->courseProgram->name }}
                    —
                    {{ $practice->module->courseLevel->name }}
                    —
    ...
    <form
        action="{{ route('admin.course-management.practice-reviews.update', $attempt) }}"
        method="POST"
        class="space-y-5">
        @csrf
        @method('PUT')
