@props([
'title' => 'News Title',
'category' => 'Education Tips',
'date' => 'Oct 18, 2023',
'excerpt' => 'Short article summary goes here.',
'image' => 'https://placehold.co/800x500/e8eef8/1e293b?text=News',
'href' => '#',
'target' => null,
'rel' => null,
'buttonText' => 'Read More',
'isExternal' => false,
])

<div {{ $attributes->merge(['class' => 'group motion-card flex h-full flex-col overflow-hidden rounded-[18px] border border-slate-200 bg-white shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-md']) }}>
    <a
        href="{{ $href }}"
        @if ($target) target="{{ $target }}" @endif
        @if ($rel) rel="{{ $rel }}" @endif
        class="block shrink-0">
        <div class="aspect-[16/9] overflow-hidden bg-slate-100">
            <img
                src="{{ $image }}"
                alt="{{ $title }}"
                class="motion-image h-full w-full object-cover">
        </div>
    </a>

    <div class="flex flex-1 flex-col p-4">
        <div class="flex items-start justify-between gap-3">
            <span class="inline-flex max-w-[62%] items-center rounded-md bg-[#f8efcf] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--color-brand-gold)]">
                <span class="truncate">
                    {{ $category }}
                </span>
            </span>

            <span class="shrink-0 pt-1 text-xs font-medium text-slate-400">
                {{ $date }}
            </span>
        </div>

        @if ($isExternal)
        <div class="mt-2">
            <span class="inline-flex rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-[#2457E6]">
                External
            </span>
        </div>
        @endif

        <div class="mt-3 flex flex-1 flex-col">
            <a
                href="{{ $href }}"
                @if ($target) target="{{ $target }}" @endif
                @if ($rel) rel="{{ $rel }}" @endif>
                <h3 class="line-clamp-2 text-xl font-bold leading-tight text-slate-900 transition hover:text-[var(--color-brand-blue)]">
                    {{ $title }}
                </h3>
            </a>

            <p class="mt-3 line-clamp-2 text-sm leading-6 text-slate-500">
                {{ $excerpt }}
            </p>

            <div class="mt-auto pt-4">
                <a
                    href="{{ $href }}"
                    @if ($target) target="{{ $target }}" @endif
                    @if ($rel) rel="{{ $rel }}" @endif
                    class="inline-flex items-center gap-2 text-sm font-bold text-[#2457E6]">
                    <span>{{ $buttonText }}</span>

                    <svg xmlns="http://www.w3.org/2000/svg" class="motion-link-arrow h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        @if ($isExternal)
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 17L17 7M17 7H9M17 7v8" />
                        @else
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M13 5l7 7-7 7" />
                        @endif
                    </svg>
                </a>
            </div>
        </div>
    </div>
</div>